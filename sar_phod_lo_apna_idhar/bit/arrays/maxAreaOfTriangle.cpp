...
int solve(vector<string> &A) {
    ...
    for(int col=0;col<m;col++){
        if(inCol[col][0][0]!=-1&&inCol[col][1][1]!=-1){
            maxarea = max(maxarea, ((int)((max(abs(inCol[col][0][0]-inCol[col][1][1])+1,abs(inCol[col][1][0]-inCol[col][0][1])+1)*max(abs(col-lb)+1,abs(rb-col)+1)+1)/2)));
        }
        if(inCol[col][0][0]!=-1&&inCol[col][2][1]!=-1){
            maxarea = max(maxarea, (max(abs(inCol[col][0][0]-inCol[col][2][1])+1,abs(inCol[col][2][0]-inCol[col][0][1])+1)*max(abs(col-lg)+1,abs(rg-col)+1)+1)/2);
        }
        if(inCol[col][1][0]!=-1&&inCol[col][2][1]!=-1){
            maxarea = max(maxarea, (max(abs(inCol[col][1][0]-inCol[col][2][1])+1,abs(inCol[col][2][0]-inCol[col][1][1])+1)*max(abs(col-lr)+1,abs(rr-col)+1)+1)/2);
        }
    }
    return maxarea;
}
